...
- **v6.0 (current):** [MIMO_Deployment_v6.0.zip](MIMO_Deployment_v6.0.zip) → **Download raw file**, then unzip.
  - Sheet 14: dump status is on each suggestion MML (Live dump / Dump status / Enabled? / Action). No separate Section 2 incon table.
  - Section 2 = Enable/Fix trial list. Section 3 = counters/KPI. Sheet 15 = CR01 10-site CME pack.
- Or [MIMO_Deployment_v6.0.xlsx](MIMO_Deployment_v6.0.xlsx) via **Download raw file**
...
| 14 | MIMO Suggest + Incon (v6.0) | Section 1 FPD boxes (MML + dump Live/status/Enabled?/Action + Counter monitor / Impact on KPI / short Notes / Jump to Basic) · Section 2 Enable/Fix proposal · Section 3 counters/KPI |
| 15 | CR01 MIMO Exec Pack | 10 gNB × 3 cells CME Proposed vs live + site-specific MML (101/102/103) |

Every MML table uses a **Seq** column (activation `n.1, n.2, …` then deactivation `n.D.*`), matching the CA workbook. **v6.0** puts dump enabled-or-not on each suggestion MML (no separate incon section). Every step sheet still ends with **Performance and Monitoring Counter**.

## How to regenerate

```bash
python3 tools/build_mimo_workbook.py          # v1 + v2.0 + v3.0 + v4.0 + v6.0
